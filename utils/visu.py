...
The code here is taken from https://github.com/hadim/keras-toolbox
...
def plot_weights(model, layer_id, n=64, ax=None, **kwargs):
    ...
    title_string = "Layer #{} called '{}' of type {}"
    
    if 'epoch' in kwargs.keys():
        title_string = title_string + " epoch #"+ str(kwargs['epoch'])
        del kwargs["epoch"]
    ...
    ax.set_title(title_string.format(layer_id, layer.name, layer.__class__.__name__))
...
def plot_feature_map(model, layer_id, X, n=256, n_columns=3, ax=None, **kwargs):
    ...
    for i, feature_map in enumerate(activations):
        ...
        im = ax.imshow(mosaic, **kwargs)
        ax.set_title(title_string.format(i, layer_id,
                                                                                  layer.name,
                                                                                  layer.__class__.__name__))
